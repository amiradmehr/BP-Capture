    ...
    while True:
        ...
        try:
            while not command_queue.empty():
                ...
                # Send the command appended with a newline (as expected by Arduino)
                arduino_serial.write((command + "\n").encode('utf-8'))
                print("Sent command:", command)
                # Optional: add a small delay to avoid flooding the Arduino
                time.sleep(0.05)
    ...
    def toggle_streaming(self):
        if self.start_stop_button.isChecked():
            if self.connection_status.value == 1:
                ...
                filename = os.path.join(data_folder, f"adcreading{now}.csv")
                try:
                    self.csv_file = open(filename, "w", newline="")
                    self.csv_writer = csv.writer(self.csv_file)
                    self.csv_writer.writerow(["Sample Index", " EXG Channel 0"])
